...
int main (void)
{
    ...
    for (;;)
    {
        ...
        commandCode = 0x51;
        i2c_master_transmit(&i2c, &commandCode, 1, SLAVE_ADDR, I2C_SR_EN);

        i2c_master_receive(&i2c, &Len, 1, SLAVE_ADDR, I2C_SR_EN);

        commandCode = 0x52;
        i2c_master_transmit(&i2c, &commandCode, 1, SLAVE_ADDR, I2C_SR_EN);

        i2c_master_receive(&i2c, rcv_buf, Len, SLAVE_ADDR, I2C_SR_DI);
