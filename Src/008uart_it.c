...
static void init_uart ()
{
    ...
    uart.UARTConf.uartParityControl = UART_PARITY_EN_EVEN;
    uart.UARTConf.uartStopBits = UART_STOP_BITS_1;
    uart.UARTConf.uartWordLength = UART_WORD_9_BITS;
...
int main (void)
{
    ...
    uart_irq_config(IRQ_NO_UART1, ENABLE);
    ...
    for (;;)
    {
        ...
        uart_interrupt_transmit(&uart, (uint8_t*) tr_buf, strlen(tr_buf));

        uart_interrupt_receive(&uart, (uint8_t*) rcv_buf, 1);
